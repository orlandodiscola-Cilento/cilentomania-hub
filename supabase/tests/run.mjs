...
try {
 ...
 // Restaurant extensions run only in this ephemeral local database.
 await identity(uid(1));
 await sql('select hub_api.save_draft($1,1,$2::jsonb)',[r2,JSON.stringify({content:{name:'Local restaurant',short_description:'Test only'},restaurant:{indoor_seats:0,accessible:null,cuisine:['Cilentana']},amenities:[{code:'vegan_options',available:true}]})]);
 const food=await val('select hub_api.get_listing($1) as v',[lid(2)]);
 check(JSON.stringify(food).includes('Cilentana'),'restaurant details returned by private API');
 check(await val('select indoor_seats as v from hub_private.restaurants where revision_id=$1',[r2])===0,'restaurant zero seats preserved');
 check(await val('select accessible as v from hub_private.restaurants where revision_id=$1',[r2])===null,'restaurant unknown remains null');
 await deny('select hub_api.save_draft($1,2,$2::jsonb)','restaurant rejects accommodation fields',[r2,JSON.stringify({accommodation:{rooms:1}})]);
 await deny('select hub_api.save_draft($1,2,$2::jsonb)','restaurant rejects accommodation services',[r2,JSON.stringify({amenities:[{code:'pool',available:true}]})]);
 await deny('select hub_api.save_draft($1,2,$2::jsonb)','restaurant rejects administrative fields',[r2,JSON.stringify({restaurant:{subscription_id:'forged'}})]);
 await identity(uid(3));
 check(await val('select count(*)::int as v from hub_private.restaurants')===0,'other organization cannot read restaurant details');
 await deny('select hub_api.save_draft($1,2,$2::jsonb)','other organization cannot edit restaurant',[r2,JSON.stringify({restaurant:{indoor_seats:9}})]);
 await identity(uid(1));await sql('select hub_api.submit_revision($1,2)',[r2]);
 await identity(uid(4));await sql("select hub_api.review_revision($1,3,'in_review')",[r2]);await sql("select hub_api.review_revision($1,4,'approved')",[r2]);
 await identity(null,'anon');check(await val("select hub_api.public_listing('test-listing-2') as v")===null,'restaurant approval does not publish');
 await deny('select * from hub_private.restaurants','anonymous restaurant table access denied');
 await identity(uid(5));await sql('select hub_api.publish_revision($1,$2,2)',[lid(2),r2]);
 await identity(null,'anon');const publicFood=await val("select hub_api.public_listing('test-listing-2') as v");
 check(publicFood.features.indoor_seats===0&&publicFood.features.accessible===null,'public restaurant features preserve structured values');
 check(!JSON.stringify(publicFood).match(/subscription|organization_id|revision_id/),'restaurant projection excludes private identifiers and commerce');
 await identity(uid(1));const newFood=await val('select hub_api.create_working_revision($1,3) as v',[lid(2)]);
 check(await val('select indoor_seats as v from hub_private.restaurants where revision_id=$1',[newFood])===0,'new restaurant revision copies features');
 await sql('select hub_api.save_draft($1,1,$2::jsonb)',[newFood,JSON.stringify({restaurant:{indoor_seats:60}})]);
 check((await val("select hub_api.public_listing('test-listing-2') as v")).features.indoor_seats===0,'restaurant draft does not change public version');
 await root();await deny('update hub_private.restaurants set indoor_seats=99 where revision_id=$1','approved restaurant details frozen',[r2]);
 await root();
 await sql("insert into hub_private.listings(id,organization_id,content_owner,type,slug) values($1,$2,$2,'accommodation','atomic-local-test')",[lid(4),oid(1)]);
 await identity(uid(1));
 await deny('select hub_api.save_listing_draft($1,1,null,null,$2::jsonb)','failed first save rolls back draft creation',[lid(4),JSON.stringify({content:{plan_id:'forged'}})]);
 check((await val('select hub_api.get_listing($1) as v',[lid(4)])).working_revision_id===null,'failed save leaves no working revision');
 const frameBase=await val('select hub_api.get_listing($1) as v',[lid(1)]);
 const frameMedia=frameBase.workingRevision.media.map(m=>({id:m.id,role:m.role,caption:m.caption,alt:m.alt,focal_x:20,focal_y:80}));
 await sql('select hub_api.save_listing_draft($1,$2,$3,$4,$5::jsonb)',[lid(1),frameBase.version,frameBase.workingRevision.id,frameBase.workingRevision.version,JSON.stringify({media:frameMedia})]);
 const framed=await val('select hub_api.get_listing($1) as v',[lid(1)]);
 check(framed.workingRevision.media[0].focalX===20&&framed.workingRevision.media[0].focalY===80,'private photo framing saved per revision');
 check(framed.publishedVersion.media[0].focalX===50,'published photo framing unchanged');
 await deny('select hub_api.save_listing_draft($1,$2,$3,$4,$5::jsonb)','invalid focal point rejected by database',[lid(1),framed.version,framed.workingRevision.id,framed.workingRevision.version,JSON.stringify({media:frameMedia.map(m=>({...m,focal_x:101}))})]);
 check((await val('select hub_api.media_capabilities() as v')).upload===true,'photo upload capability requires enabled identity');
 const onlineBefore=await val('select hub_api.get_listing($1) as v',[lid(1)]);
 const onlineArgs=[lid(1),onlineBefore.version,onlineBefore.workingRevision.id,onlineBefore.workingRevision.version];
 const onlineSaved=await val('select hub_api.save_listing_draft($1,$2,$3,$4,$5::jsonb) as v',[...onlineArgs,JSON.stringify({content:{claim:'Frase online'},accommodation:{rooms:0}})]);
 check(onlineSaved.workingRevision.claim==='Frase online','online atomic save retains presentation text');
 check(onlineSaved.publishedVersion.claim===null,'online save leaves published presentation unchanged');
 await deny('select hub_api.save_listing_draft($1,$2,$3,$4,$5::jsonb)','online stale revision rejected',[...onlineArgs,'{}']);
 await identity(uid(3));await deny('select hub_api.save_listing_draft($1,$2,$3,$4,$5::jsonb)','online other organization denied',[...onlineArgs,'{}']);
 await identity(uid(1));check(Array.isArray(await val('select hub_api.list_municipalities() as v')),'online municipality catalog available to enabled user');
 await root();
