-- Private photo framing support. No customer data or Auth/Storage DDL.
alter table hub_private.listing_revision_media
 add column focal_x numeric not null default 50 check(focal_x between 0 and 100),
 add column focal_y numeric not null default 50 check(focal_y between 0 and 100);
...
begin
 ...
 if patch ? 'media' then
  ...
  for item in select value from jsonb_array_elements(patch->'media') loop
   perform hub_private.assert_keys(item,array['id','role','caption','alt','focal_x','focal_y']);
   insert into hub_private.listing_revision_media(revision_id,listing_id,organization_id,media_id,role,position,caption,alt,focal_x,focal_y) values(rid,r.listing_id,r.organization_id,(item->>'id')::uuid,item->>'role',ord,item->>'caption',coalesce(item->>'alt',''),coalesce((item->>'focal_x')::numeric,50),coalesce((item->>'focal_y')::numeric,50));
   ord:=ord+1;
  end loop;
 end if;
 return r.version+1;
...
begin
 ...
 if r.id is null then
  ...
  insert into hub_private.listing_revision_media select new_id,listing_id,organization_id,media_id,role,position,caption,alt,focal_x,focal_y from hub_private.listing_revision_media where revision_id=r.id;
 end if;
...
create or replace function hub_private.revision_document(rid uuid) returns jsonb language sql stable set search_path='' as $$
 ...
 'media',coalesce((select jsonb_agg(jsonb_build_object('id',m.id,'storagePath',m.storage_path,'role',rm.role,'position',rm.position,'caption',rm.caption,'alt',rm.alt,'focalX',rm.focal_x,'focalY',rm.focal_y) order by rm.position)
...
language sql stable security definer set search_path='' as $$
 select jsonb_build_object(
  ...
  'media',coalesce((select jsonb_agg(jsonb_build_object('id',m.id,'bucket','listing-published','path',m.published_path,'role',rm.role,'position',rm.position,'caption',rm.caption,'alt',rm.alt,'focalX',rm.focal_x,'focalY',rm.focal_y) order by rm.position)
...
create function hub_api.media_capabilities() returns jsonb language plpgsql stable security definer set search_path='' as $$
begin
 if hub_api.current_context() is null then raise exception 'not_authorized'; end if;
 return '{"upload":true,"focal":true}'::jsonb;
end $$;
alter function hub_api.media_capabilities() owner to hub_executor;
revoke all on function hub_api.media_capabilities() from public,anon,authenticated,service_role;
grant execute on function hub_api.media_capabilities() to authenticated;
