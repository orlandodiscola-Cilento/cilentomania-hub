...
module.exports=dir=>{
 ...
 assert.deepEqual(ops,['operatori/.htaccess','operatori/THIRD-PARTY-NOTICES.txt','operatori/config.json','operatori/css/area-operatori.css','operatori/index.html','operatori/js/app.js','operatori/js/password.js','operatori/password.html'].sort());
 ...
 for(const page of ['app','password']){const js=fs.readFileSync(path.join(dir,'operatori/js',page+'.js'),'utf8');assert.ok(!/anna@example|demo-repository|indexedDB|cilentomania_operator_demo_user/.test(js));assert.match(js,/https:\/\/www\.cilentomania\.it\/hub\/operatori\/password\.html/);}
