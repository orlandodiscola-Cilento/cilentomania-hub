...
(async()=>{
 ...
 global.document={createElement:()=>({getContext:()=>({drawImage(){}}),toBlob:cb=>cb(new Blob(['candidate'],{type:'image/webp'}))})};
 ...
 }}),storage:{from:bucket=>{assert.equal(bucket,'listing-drafts');return {upload:async(path,file,options)=>{uploads++;if(path.endsWith('/original'))assert.equal(file,blob);else{assert.ok(path.endsWith('/candidate'));assert.equal(file.type,'image/webp');assert.notEqual(file,blob);}assert.equal(options.upsert,false);return failUpload?{error:{statusCode:'503'}}:{data:{path}};},createSignedUrl:async()=>({data:{signedUrl:'https://qgkwqzjapvjvzmvdfges.supabase.co/storage/v1/object/sign/listing-drafts/test'}})}}}};
 ...
 failUpload=false;args.revisionVersion=2;const saved=await repo.save({},args);assert.equal(registered,1,'retry reuses registered asset');assert.equal(uploads,3);assert.equal(saved.workingRevision.media[0].focalX,25);assert.equal(saved.workingRevision.media[0].focalY,75);assert.equal(saved.publishedVersion,null);
