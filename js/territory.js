...
async function loadMunicipalityModuleData(type){
 // Refresh remote publication status and expiring image links whenever the list opens.
 ...
 try{const online=await globalThis.HubPublicListings?.list(type)||[];municipalityModuleDataCache[type].push(...online);}catch{console.warn('Catalogo online temporaneamente non disponibile.');}
...
function municipalityPhotoPosition(item){return [item.focalX,item.focalY].map(v=>(Number.isFinite(v)?Math.max(0,Math.min(100,v)):50)+'%').join(' ');}
...
function municipalityModuleFilterHtml(config,records,filters={}){
 const isShowcase=['accommodation','restaurant'].includes(config.entityType);
 ...
 const categoryMarkup=isShowcase&&categories.length
 ...
 const quickStart=isShowcase?'<details class="module-sleep-extra-filters"'+(booleanSelection.length?' open':'')+'><summary>'+safeTerritoryText(territoryTranslate('hospitality.quickFilters','Filtri rapidi'))+'</summary>':'<div class="module-filter-quick"><p class="module-filter-quick-title">'+safeTerritoryText(territoryTranslate('hospitality.quickFilters','Filtri rapidi'))+'</p>';
 const quickEnd=isShowcase?'</details>':'</div>';
...
function municipalityModuleSleepHeroHtml(municipalityName,config){
 ...
 const imageMarkup=coverRecord
  ...
  :'';
 return '<header class="module-sleep-hero"><div class="module-sleep-hero__media">'+imageMarkup+'<div class="module-sleep-hero__fallback" aria-hidden="true"></div><div class="module-sleep-hero__shade" aria-hidden="true"></div><div class="module-sleep-hero__content"><p class="module-sleep-eyebrow">'+safeTerritoryText(isEat?territoryTranslate('modules.eat','Dove mangiare'):territoryTranslate('modules.sleep','Dove dormire'))+'</p><h1 class="module-sleep-hero__title">'+safeTerritoryText(municipalityName)+'</h1><p>'+safeTerritoryText(isEat?territoryTranslate('hospitality.eatTagline','Il Cilento, da assaporare.'):territoryTranslate('hospitality.sleepTagline','Il tuo prossimo risveglio nel Cilento.'))+'</p></div></div>'+(intro?'<details class="module-sleep-story"><summary>'+safeTerritoryText(territoryTranslate('hospitality.discoverTown','Scopri'))+' '+safeTerritoryText(municipalityName)+'</summary><p>'+safeTerritoryText(intro)+'</p></details>':'')+'</header>';
}
function municipalityModuleSleepConciergeHtml(config){
 const isEat=config.entityType==='restaurant';
 const name=isEat?'Cilentino Chef':'Cilentino Concierge';
 return '<section class="module-sleep-concierge" aria-label="'+name+'"><img class="module-sleep-concierge__avatar" src="assets/cileo/avatar/'+(isEat?'cilentino-chef.png':'cilentino-concierge.png')+'" alt="'+name+'" loading="lazy"><div><strong>'+name+'</strong><p>'+safeTerritoryText(isEat?territoryTranslate('hospitality.eatConciergeIntro','Ti aiuto a scegliere dove mangiare e a scoprire i sapori del Cilento.'):territoryTranslate('hospitality.sleepConciergeIntro','Ti aiuto a trovare la struttura piu adatta al tuo soggiorno.'))+'</p><button type="button" class="module-sleep-chat" data-module-concierge>'+safeTerritoryText(territoryTranslate('hospitality.askCilentino','Chiedi a Cilentino'))+' <span aria-hidden="true">↗</span></button></div></section>';
}
function municipalityModuleViewHtml(type,municipalityName,comuneId,records,filters={}){
 ...
 const concierge=isShowcase?municipalityModuleSleepConciergeHtml(config):'';
...
function municipalityModuleDetailHtml(item,config,municipalityName,comuneId){
 ...
 if(isEat&&typeof item.aperto_pranzo==='boolean')pushPractical(territoryTranslate('hospitality.openAtLunch','Aperto a pranzo'),municipalityBooleanLabel(Boolean(item.aperto_pranzo)));
 if(isEat&&typeof item.aperto_cena==='boolean')pushPractical(territoryTranslate('hospitality.openAtDinner','Aperto a cena'),municipalityBooleanLabel(Boolean(item.aperto_cena)));
 const contactRows=[];
...
async function openMunicipalityModule(type,municipalityName,comuneId,state={}){
 ...
 const title=config.titlePrefix+' '+municipalityName;
 openPanel(['sleep','eat'].includes(type)?'':title,municipalityModuleViewHtml(type,municipalityName,comuneId,filteredRecords,initialFilters));
 overlay.classList.toggle('municipality-module-sleep-open',['sleep','eat'].includes(type));
 if(['sleep','eat'].includes(type))configureNavigationDock('modal-return');
...
async function openMunicipalityDetailById(type,municipalityName,comuneId,itemId,navigationState){
 ...
 if(type==='sleep'&&globalThis.OperatorProfile){
  ...
  const hero=panelContent.querySelector('.op-hero-background');if(hero&&Number.isFinite(item.focalX)&&Number.isFinite(item.focalY))hero.style.objectPosition=item.focalX+'% '+item.focalY+'%';
