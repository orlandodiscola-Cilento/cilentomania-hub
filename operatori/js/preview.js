...
async function start(){
 const params=new URLSearchParams(location.search),admin=params.get('admin')==='1';const {repo,auth}=await getBackend();const session=await auth.current({area:admin?'admin':'operator'});
