...
export async function mountReview(root,repo,session,back) {
 ...
 root.innerHTML=`<h1>Revisioni delle schede</h1><button id="back-online">← Le mie schede</button><p role="status" class="status"></p><div class="queue">${rows.map(r=>`<article class="panel"><h2>${e(r.name)}</h2><p>${e(r.organizationName)} · ${e(r.isPublished?'Pubblicata':labels[r.status])}</p><div class="actions"><button data-compare="${e(r.id)}">Confronta</button>${r.status==='submitted'?`<button data-target="in_review" data-id="${e(r.id)}">Avvia revisione</button>`:''}${r.status==='in_review'?`<button data-target="approved" data-id="${e(r.id)}">Approva</button><button data-target="changes_requested" data-id="${e(r.id)}">Richiedi modifiche</button>`:''}${r.status==='approved'&&r.isCurrent&&!r.isPublished&&context.user.role==='cilentomania_admin'?`<button data-prepare="${e(r.id)}">Verifica foto</button><button class="primary" data-publish="${e(r.id)}">Pubblica scheda</button>`:''}</div><div data-comparison="${e(r.id)}"></div></article>`).join('')||'<p>Nessuna revisione disponibile.</p>'}</div><p class="footnote">L’approvazione non pubblica automaticamente la scheda.</p>`;
 ...
 root.querySelectorAll('[data-prepare],[data-publish]').forEach(b=>b.onclick=async()=>{
  const r=rows.find(r=>r.id===(b.dataset.prepare||b.dataset.publish));
  if(b.dataset.publish&&!await ask('Pubblicare questa scheda approvata? Le foto saranno prima verificate. Le schede dimostrative rimangono escluse dagli elenchi turistici.'))return;
  b.disabled=true;const status=root.querySelector('[role=status]');status.textContent='Verifica delle foto in corso…';
  try{if(b.dataset.publish){await repo.publish(session,{id:r.listingId,revisionId:r.id});await mountReview(root,repo,session,back);root.querySelector('[role=status]').textContent='Scheda pubblicata.';}
   else{await repo.prepareMedia(session,r.listingId);status.textContent='Foto verificate e pronte. La scheda non è stata pubblicata.';}
  }catch(err){status.textContent=err.message||'Operazione non riuscita. Nessuna pubblicazione completata.';}finally{b.disabled=false;}
 });
