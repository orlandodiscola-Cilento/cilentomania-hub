...
export function accessShell(root,title,fields,buttons) {
  root.innerHTML=`<section class="panel login"><div class="welcome"><p class="eyebrow">Cilentomania HUB</p><h1>La tua ospitalità,<br>in buone mani.</h1><p>Uno spazio semplice per raccontare la tua attività e tenere aggiornata la tua scheda.</p></div><form><p class="eyebrow">Area Operatori</p><h2></h2>${back}${fields}${buttons}<p role="status" class="status" aria-live="polite"></p></form></section>`;
